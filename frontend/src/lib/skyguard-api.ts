...
export interface ShapFeature { feature:string; shap_value:number; anomaly_contribution:number; anomaly_contribution_pct:number; direction:string }
export interface Xai { summary:string; primary_ml_driver?:string|null; details:string; recommended_action:string; top_evidence:{name:string;score:number}[]; shap?:{available:boolean;method?:string;top_feature?:string|null;features?:ShapFeature[];reason?:string}|null; temporal_score:number; spatial_score:number; spatial_context?:Record<string,unknown>; baseline?:Record<string,number>|null; corrected_values?:{temperature:number;pressure:number;humidity:number}|null; communication_gap_seconds?:number|null }
export interface Reading { id:number; station_id:string; timestamp:string; temperature:number; pressure:number; humidity:number; anomaly:boolean; anomaly_score:number; confidence:number; severity:Severity; root_cause:string; explanation:string; corrected_values:{temperature:number;pressure:number;humidity:number}|null; shap_values?:{available:boolean;features?:ShapFeature[];top_feature?:string|null}|null; xai?:Xai|null; source?:string|null; device_id?:string|null }
...
export interface Analytics { station_id:string; seasonal_baseline:{temperature:number;pressure:number;humidity:number;sample_count:number}|null; maintenance_forecast:{degradation_risk:number;maintenance_priority:string;recommendation:string;anomaly_rate:number;recent_anomaly_rate:number;samples_analyzed:number}; latest:Reading|null }
...
export const api={
 ...
 trend:(id:string,hours=1)=>request<Reading[]>(`/stations/${encodeURIComponent(id)}/trend?hours=${hours}`),
 analytics:(id:string)=>request<Analytics>(`/stations/${encodeURIComponent(id)}/analytics`),
