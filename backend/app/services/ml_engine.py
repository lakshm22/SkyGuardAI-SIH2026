...
from app.services.explainability import shap_explanation

FEATURES = ["temperature", "pressure", "humidity"]


class AnomalyEngine:
    """Hybrid multivariate detector with SHAP-based ML explainability."""
    def __init__(self, model_path="./models/isolation_forest.joblib"):
        self.model_path = Path(model_path)
        self.model_path.parent.mkdir(parents=True, exist_ok=True)
        self.scaler = StandardScaler()
        self.model = IsolationForest(n_estimators=250, contamination=0.02, random_state=42)
        self._train_or_load()
    ...
    def domain_flags(self, t, p, h):
        flags = {
            "temperature_range": max(0, abs(t - 29) / 14),
            "pressure_deviation": max(0, abs(p - 1008) / 12),
            "humidity_extreme": max(0, abs(h - 65) / 45),
            "cross_parameter": float((t > 45 and h > 85) or (t < -5 and h > 90)),
        }
        return {k: float(np.clip(v, 0, 1)) for k, v in flags.items()}

    def analyze(self, t, p, h, temporal=0.0, spatial=0.0, frozen=0.0,
                regional_event=False, explain=True):
        ...
        components = {
            ...
            "Temporal/seasonal deviation": temporal,
            "Frozen value": frozen,
        }
        weighted = (
            0.38 * ml + 0.16 * flags["temperature_range"] + 0.10 * flags["pressure_deviation"] +
            0.09 * flags["humidity_extreme"] + 0.12 * spatial + 0.10 * temporal + 0.05 * frozen
        )
        domain_floor = max(
            0.80 * flags["temperature_range"], 0.72 * flags["pressure_deviation"],
            0.82 * flags["humidity_extreme"], 0.90 * flags["cross_parameter"],
            0.72 * max(temporal, frozen)
        )
        score = float(np.clip(max(weighted, domain_floor), 0, 1))
        if regional_event and score >= 0.60:
            score = float(np.clip(score * 0.82, 0, 1))
        anomaly = score >= 0.60
        ...
        root = self._root_cause(ranked, t, p, h, spatial, frozen, regional_event)
        shap_info = shap_explanation(self.model, self.scaler, t, p, h) if explain else {"available": False}
        return score, confidence, severity, anomaly, root, components, shap_info

    def _root_cause(self, ranked, t, p, h, spatial, frozen, regional_event):
        if regional_event:
            return "Probable regional meteorological event"
        if frozen > 0.70:
            return "Frozen sensor / repeated value"
        top = ranked[0][0]
        ...
        if top == "Temporal/seasonal deviation": return "Temporal/seasonal pattern deviation"
